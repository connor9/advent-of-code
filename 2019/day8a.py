...
# the first digit corresponds to the top-left pixel of the first layer, the second digit corresponds to the pixel to
# the right of that on the same layer, and so on until the last digit, which corresponds to the bottom-right pixel of
# the last layer.
#
...
for i in range(data_size // (w*h)):
    ...
    if cnt < min_cnt:
        ...
        min_cnt = cnt
        min_layer = i
        min_calc = cnt_1*cnt_2

print(min_layer, min_layer+1, min_cnt, min_calc)
