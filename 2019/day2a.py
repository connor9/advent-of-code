...
def run_program(memory):
    ...
    while True:
        opcode = memory[instruction_counter]

        if opcode == 99:
            break

        mem_a = memory[instruction_counter + 1]
        mem_b = memory[instruction_counter + 2]
        mem_c = memory[instruction_counter + 3]
        if opcode == 1:
            memory[mem_c] = memory[mem_a] + memory[mem_b]
        elif opcode == 2:
            memory[mem_c] = memory[mem_a] * memory[mem_b]
...
for test in test_data:
    output_memory = run_program(test[0])
    if output_memory != test[1]:
        print("Test failed!")
        import sys
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    instructions_data[1] = 12
    instructions_data[2] = 2

    output_memory = run_program(instructions_data)
    print("Value of position 0:", output_memory[0])
